...
router.post(`${parentDir}/service/generate-url`, (req, res) => {
	const { whichConfidence, whichKBV, hmrcSets, passFailKBV } = req.body; // Get the form data sent with POST
	...
	const buildUserReadout = `${buildConfidenceString} ➔ ${buildKBVString} ➔ ${buildPassFailString}` + (buildHMRCString ? ` ➔ ${buildHMRCString}` : '');

	// Construct the URI conditionally
	const startURI = req.protocol + '://' + req.get('host');
	let userURI = `${startURI}${parentDir}/customise-example?confidence=${buildConfidenceString}&kbv=${buildKBVString}&passFail=${buildPassFailString}`;
	if (buildHMRCString) {
