...
router.post(`${parentDir}/service/generate-url`, (req, res) => {
	const { whichConfidence, whichKBV, hmrcSets, passFailKBV, experianKBV, continuityIdentity } = req.body; // Get the form data sent with POST
	...
	const buildUserReadout = `${buildConfidenceString} ➔ ${buildKBVString} ➔ ${buildPassFailString} ➔ ${buildExperianKBVString} ➔ ${buildcontinuityIdentity}` + (buildHMRCString ? ` ➔ ${buildHMRCString}` : '');

	// Construct the URI conditionally
	const startURI = req.protocol + '://' + req.get('host');
	let userURI = `${startURI}${parentDir}/service/service-start?confidence=${buildConfidenceString}&kbv=${buildKBVString}&passFail=${buildPassFailString}&experianKBV=${buildExperianKBVString}&continuityIdentity=${buildcontinuityIdentity}`;
	if (buildHMRCString) {
